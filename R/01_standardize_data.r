...
# Austria
aus <- data.table(read_excel("data/data_raw/Odonata_Austria_Export.xlsx", sheet = "Tabelle1"))
aus <- setnames(aus, names(aus), c("species_scientific_name", "sp_authority", "sp_year", 
...
# Belgium
bel <- data.table(readRDS("data/data_raw/BelgieData.rds"))
bel <- setnames(bel, names(bel), c("species_scientific_name", "species_names_nl", 
                                "species_code", "date", "day", "month", "year", "utm1_mgrs"))
bel <- bel[, utm1_mgrs := paste0("31U", utm1_mgrs)]

# Needs install_github
bel[ , ":="(y_coord =mgrs_to_latlng(bel$utm1_mgrs)$lat, x_coord =mgrs_to_latlng(bel$utm1_mgrs)$lng)]
bel[, crs := 4326]
bel[, count := 1] # is this occurrence
bel[, dataset := "bel"]
bel[ , ":="(x_coord = as.numeric(x_coord), y_coord = as.numeric(y_coord))]

bel2 <- fread("data/data_raw/2024-076_libellen_EN.csv")
bel2 <- bel2[, .(name_lat, date, amount, method, 
                 lifestage, sex, lon, lat, location, source)]
setnames(bel2, names(bel2), c("species_scientific_name", "date", "count", "record_type", "stage", "sex", "x_coord",
                "y_coord", "location", "data_source"))
bel2[, ":="(day = mday(as.IDate(date, format="%Y/%m/%d")), month = month(as.IDate(date, format="%Y/%m/%d")),
        year = year(as.IDate(date, format="%Y/%m/%d")))]

bel2[, ":="(x_coord = as.numeric(gsub("\\,", "\\.", x_coord)), y_coord = as.numeric(gsub("\\,", "\\.", y_coord)))]
bel2[, dataset := "bel"]
bel <- rbind(bel, bel2, fill = TRUE)


# Cyprus
cyp1 <- fread("data/data_raw/Cyprus corrected.csv")
cyp1 <- setnames(cyp1, names(cyp1), c("grid_ref_number", "species_scientific_name", "stage", "count", "larvea",
                                "exuvia", "day", "month", "year", "y_coord", "x_coord", "record_type",
                                "accuracy", "country", "recorder", "location", "comment", "loation_name",
                                "recorders"))
cyp1[, y_coord := gsub("\\xff", "", gsub("\\,", "\\.", y_coord, useBytes = TRUE), useBytes = TRUE)]
cyp1[, x_coord := gsub("\\xff", "", gsub("\\,", "\\.", x_coord, useBytes = TRUE), useBytes = TRUE)]

lf <- list.files("data/data_raw/Cyprus Data dragonflies")
cyp2 <- data.table()
for(i in seq_along(lf)){
    cyp_data <- data.table(read_excel(file.path("data", "data_raw","Cyprus Data dragonflies", lf[i]), sheet = "Sheet1"))
    cyp_data <- setnames(cyp_data, names(cyp_data), c("grid_ref_number", "species_scientific_name", "date", "day", "month", "year", "stage", "count", "larvea",
                    "exuvia", "x_coord", "y_coord", "longitude_ms", "latitude_ms", "location_name", "altitutde_m"))
    cyp_data[, y_coord := gsub("\\xff", "", gsub("\\,", "\\.", y_coord, useBytes = TRUE), useBytes = TRUE)]
    cyp_data[, x_coord := gsub("\\xff", "", gsub("\\,", "\\.", x_coord, useBytes = TRUE), useBytes = TRUE)]
...
ned <- setnames(ned, names(ned), c("record_url", "species_names_nl", "year", "month", "day", "date_scale", "x", "y", 
                            "location_scale", "count", "stage", "data_source"))

ned_coord <- data.table(st_coordinates(st_transform(st_as_sf(ned, coords = c("x", "y"), crs = 28992), 4326)))
ned[, ":="(x_coord = ned_coord$X, y_coord=ned_coord$Y, crs=4326)]
sp_ned <- fread("data/data_raw/Soortcodes_nl_sci.csv")
setnames(sp_ned, c("srt_scientific","srt_Nednaam"), c("species_scientific_name", "species_names_nl"))
sp_ned[, species_names_nl := tolower(species_names_nl)]
ned <- merge(ned, sp_ned[, .(species_scientific_name, species_names_nl)], by = "species_names_nl", all.x = TRUE)
ned[, dataset := "ned"]

# France STELLI
fr_steli <- fread("data/data_raw/STELI_data_FR_DMS.csv")
fr_steli <- fr_steli[rang %in% c("ES", "SSES"), c("taxon", "date", "duree", "insee_commune", "nom_commune", 
...
fr_steli[, ":="(day = mday(as.IDate(date, format="%d/%m/%Y")), month = month(as.IDate(date, format="%d/%m/%Y")), year = year(as.IDate(date, format="%d/%m/%Y")))]
fr_steli[, dataset := "fr_steli"]

# France
fr <- fread("data/data_raw/France Opportunistics data (Opie)/odonata_202410091558.csv")
fr <- fr[, .(taxon, latitude, longitude, commune, date, observateur)]
sp_list <- data.table(taxon =fr[, unique(taxon)])
sp_list[, species_scientific_name := str_trim(gsub("\\(", "", 
                                        substr(sp_list$taxon, 1, 
                                        sapply(gregexpr("[A-Z]", sp_list$taxon), `[`, 2)-1)))]
fr <- merge(fr, sp_list, by = "taxon")
setnames(fr, names(fr),c("species_scientific_names_long", "y_coord", "x_coord", "region_nae", "date", 
            "recorder_name","species_scientific_name"))
fr[, ":="(day = mday(as.IDate(date)), month = month(as.IDate(date)), year = year(as.IDate(date)))]
